...
def get_block(size):
    ...
    rect = pygame.Rect(96, 64, size, size)  # Xác định vùng khối vuông trong ảnh nền
...
def start_menu(window, background_image_path, button_position1):
    ...
    while running:
        ...
        draw_text(window, '''WELCOME TO UEH GARDEN''', WIDTH // 2, 100, center=True, color = white, size = 99) # Sửa lại vị trí sau khi import ảnh nền của Hằng vào 
...
def menu_map(window, background_image_path, button_position2):
    ...
    button_width, button_height = 270, 210
    button_hover_color = (0, 255, 0)
    # Load button image and scale
    button_image = pygame.image.load("assets/Background/san vđ ueh.png")
...
def menu_map2(window, background_image_path, button_position2):
    ...
    button_hover_color = (0, 255, 0)
    # Load button image and scale
    button_image = pygame.image.load("assets/Background/toa nha ueh.png")
    button_image = pygame.transform.scale(button_image, (button_width, button_height))
...
def menu_map3(window, background_image_path, button_position2):
    ...
    # Define button properties
    button_width, button_height = 400, 280
    button_hover_color = (0, 255, 0)
    # Load button image and scale
    button_image = pygame.image.load("assets\Background\san bong da.png")
    button_image = pygame.transform.scale(button_image, (button_width, button_height))
...
def menu_map4(window, background_image_path, button_position2):
    ...
    button_hover_color = (0, 255, 0)
    # Load button image and scale
    button_image = pygame.image.load("assets\Background\green campus.png")
    button_image = pygame.transform.scale(button_image, (button_width, button_height))
...
def handle_move(player, object):
    ...
    collide_left = collide(player, object, -PLAYER_VEL * 3)  # Kiểm tra va chạm bên trái
    collide_right = collide(player, object, PLAYER_VEL * 3)  # Kiểm tra va chạm bên phải
...
class CustomBlock(pygame.sprite.Sprite):
    def __init__(self, x, y, size, image_path):
        super().__init__()
        # Load and scale the image for the block
        self.image = pygame.image.load(image_path).convert_alpha()
        self.image = pygame.transform.scale(self.image, (size, size))
        
        # Set position and dimensions
        self.rect = self.image.get_rect(topleft=(x, y))
        self.size = size  # Store size for future use
        self.x, self.y = x, y  # Store position

    def change_image(self, a):
        """Change the block's image dynamically."""
        self.image = pygame.image.load("question.png").convert_alpha()
        self.image = pygame.transform.scale(self.image, (self.size, self.size))

    def draw(self, window, offset_x=0):
        """Draw the block on the screen with optional offset."""
...
def create_one_question_block_structure(x, y, block_size, orientation = '''horizontal'''):
    ...
    if orientation == '''question 1''':  # Kiểu nằm ngang
        blocks.append(MysteryBlock(x* block_size, y, block_size))
...
def main2(window):
    ...
    o=17
    custom_block = CustomBlock(200, 400, 64, 'assets\Background\hoi.png')
    ...
    mystery_block1 = create_one_question_block_structure(10, 300+o, block_size, '''question 1''')
    mystery_block2 = create_one_question_block_structure(40, 200+o, block_size, '''question 2''')
    mystery_block3 = create_one_question_block_structure(80, 350+o, block_size, '''question 3''')
    mystery_block4 = create_one_question_block_structure(100, 250+o, block_size,'''question 4''')
    mystery_block5 = create_one_question_block_structure(160, 280+o, block_size,'''question 5''') 
...
def main(window):
    class Player(pygame.sprite.Sprite):
        ...
        def draw(self, win, offset_x):
            win.blit(self.sprite, (self.rect.x - offset_x, self.rect.y))  # Vẽ nhân vật lên màn hình
    # Hàm hiển thị menu bắt đầu game
    clock = pygame.time.Clock()  # Đồng hồ game
    background, bg_image = get_background('''bgtest.jpg''')  # Lấy nền game
    block_size = 96  # Kích thước khối
    player = Player(100, 150, 50, 50)  # Tạo nhân vật
    fire = Fire(100, HEIGHT - block_size - 64, 16, 32)  # Tạo bẫy lửa
    fire.on()  # Bật bẫy lửa
    for x in range(0,3):
        if x==3:
            pygame.quit()
    ...
    floor = [Block(i * block_size, HEIGHT - block_size+5, block_size) for i in range(-WIDTH // block_size, (WIDTH * 3) // block_size)]
    ...
    while run:
                # Check if player reaches the x-axis condition
        if player.rect.x > 3000:  # Example condition
            action = trigger_menu(window, '''assets\MainCharacters\Zeenone\win.png''', (WIDTH // 2 - 100, HEIGHT // 2 + 200))
            if action == '''menu_map''':
                map_action = menu_map(window, '''assets/Background/z6105275572936_31db720c63d3dafccde4dd83d4772e28.jpg''', (WIDTH // 4 - 125, HEIGHT // 2 - 100))
                if map_action == '''return_to_main''':
                    return  # Exit `main` and restart the game loop
        clock.tick(FPS)  # Giới hạn FPS
        for event in pygame.event.get():  # Xử lý sự kiện
        ...
        if game_over:  # Xử lý màn hình Game Over
            game_over_image_path = '''assets\\MainCharacters\\Zeenone\\lose.png'''
            game_over_image = pygame.image.load(game_over_image_path).convert_alpha()
            game_over_image = pygame.transform.scale(game_over_image, (400, 300))
            window.fill((0, 0, 0))
            font = pygame.font.Font(None, 74)
        ...
        fire.loop()  # Cập nhật trạng thái bẫy lửa
        handle_move(player, objects)  # Xử lý di chuyển
        if player.rect.top > HEIGHT or player.rect.bottom < -5:  # Kiểm tra rơi khỏi màn hình
            game_over = True
            continue
        draw(window, background, bg_image, player, objects, offset_x)  # Vẽ tất cả đối tượng
        if ((player.rect.right - offset_x >= WIDTH - scroll_area_width) and player.x_vel > 0) or (
...
# Hàm hiển thị menu bắt đầu game
    ...
    while run:
                # Check if player reaches the x-axis condition
        if player.rect.x > 3500:  # Example condition
            action = trigger_menu(window, '''assets\MainCharacters\Zeen_png\Zeen_3\Zeen_3_win.png''', (WIDTH // 2 - 100, HEIGHT // 2 + 200))
            if action == '''menu_map''':
                map_action = menu_map(window, '''assets/Background/z6105275572936_31db720c63d3dafccde4dd83d4772e28.jpg''', (WIDTH // 4 - 125, HEIGHT // 2 - 100))
                if map_action == '''return_to_main''':
                    return  # Exit `main` and restart the game loop
        ...
        if game_over:  # Xử lý màn hình Game Over
            game_over_image_path = '''assets\MainCharacters\Zeen_png\Zeen_3\Zeen_3_lose.png'''
            game_over_image = pygame.image.load(game_over_image_path).convert_alpha()
            game_over_image = pygame.transform.scale(game_over_image, (400, 300))
            window.fill((0, 0, 0))
            font = pygame.font.Font(None, 74)
...
# Hàm hiển thị menu bắt đầu game
    ...
    while run:
                # Check if player reaches the x-axis condition
        if player.rect.x > 5000:  # Example condition
            action = trigger_menu(window, '''assets\MainCharacters\Zeen_png\Zeen_4\Zeen_4_win.png''', (WIDTH // 2 - 100, HEIGHT // 2 + 200))
            if action == '''menu_map''':
                map_action = menu_map(window, '''assets/Background/z6105275572936_31db720c63d3dafccde4dd83d4772e28.jpg''', (WIDTH // 4 - 125, HEIGHT // 2 - 100))
                if map_action == '''return_to_main''':
                    return  # Exit `main` and restart the game loop
        ...
        if game_over:  # Xử lý màn hình Game Over
            game_over_image_path = '''assets\MainCharacters\Zeen_png\Zeen_4\Zeen_4_lose.png'''
            game_over_image = pygame.image.load(game_over_image_path).convert_alpha()
            game_over_image = pygame.transform.scale(game_over_image, (400, 300))
            window.fill((0, 0, 0))
            font = pygame.font.Font(None, 74)
        ...
        fire.loop()  # Cập nhật trạng thái bẫy lửa
        handle_move(player, objects)  # Xử lý di chuyển
        if player.rect.top > HEIGHT+100 or player.rect.bottom < 0:  # Kiểm tra rơi khỏi màn hình
            game_over = True
            continue
        draw(window, background, bg_image, player, objects, offset_x)  # Vẽ tất cả đối tượng
        if ((player.rect.right - offset_x >= WIDTH - scroll_area_width) and player.x_vel > 0) or (
                (player.rect.left - offset_x <= scroll_area_width) and player.x_vel < 0):
            offset_x += player.x_vel*3
    pygame.quit()
    quit()
if __name__ == "__main__":
    menu_background_path = "assets/Background/startmenu.jpg"
    start_button_position = (WIDTH // 2 - 100, HEIGHT // 2 + 250)
    map_menu = "assets/Background/z6105275572936_31db720c63d3dafccde4dd83d4772e28.jpg"
    change_picture_button_position = (WIDTH - 500, HEIGHT // 2 - 50)
    change_picture_button_position2 = (WIDTH - 460, HEIGHT // 2 - 250)
    change_picture_button_position3 = (100, 10)
    change_picture_button_position4 = (365,HEIGHT-370)
